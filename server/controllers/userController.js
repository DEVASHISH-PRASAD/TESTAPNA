...
const sendOtpInstance = new SendOtp(process.env.MSG91_API_KEY);
...
const sendOtp = (phone, otp) => {
  sendOtpInstance.send(phone, "ApnaAssistant", otp, (err, data) => {
    if (err) {
...
export const register = async (req, res, next) => {
  ...
  try {
    ...
    if (existingTempUser.length > 0) {
      return res
        .status(400)
        .json({ message: "OTP already sent to this email. Please verify." });
    }
